...
export class AppPage {
   ...
   async llenarFormularioAlbum()
  {
    ...
    await inputArray[1].sendKeys('https://images-na.ssl-images-amazon.com/images/I/71OmC6lSOcL._SL1500_.jpg');
    this.esperar(200)
    await element(by.xpath("//input[@type='date']")).sendKeys('11/05/2021');
    this.esperar(200)
    await element(by.css('textarea')).click()
    await element(by.css('textarea')).sendKeys('adgfkjhfjhadfnkebhnarfusvnauidgfbaiubgfdsvnfbahuibgfdsabgnfhuib')
    this.esperar(200)
    const selector = await element(by.xpath("//select[@aria-label='Default select example']"))
    await selector.click()
    const option = await selector.element(by.xpath('//option[@value="2: Folk"]'))
    await option.click()
    const selector2 = await element(by.xpath("//select[@aria-label='seleccion de sello']"))
    await selector2.click()
    const option2 = await selector.element(by.xpath('//option[@value="2: Elektra"]'))
    await option2.click()
  }

  async clickSubmitButton()
  {
    await browser.wait(element(by.xpath('//button[@type="submit"]')).isEnabled(),5000)
  ...
  async eliminarAlbum()
  {
    ...
    await browser.wait(element(by.css('.swal2-confirm.swal2-styled.swal2-default-outline')).isPresent(),5000);
    await element(by.css('.swal2-confirm.swal2-styled.swal2-default-outline')).click();
    await browser.wait(element(by.css('.swal2-confirm.swal2-styled')).isDisplayed(),5000);
    await element(by.css('button.swal2-confirm.swal2-styled')).click();
  ...
  {
    await browser.wait(element(by.xpath('//button[@data-target="#trackCreationModal"]')).isDisplayed(),5000)
    await element(by.xpath('//button[@data-target="#trackCreationModal"]')).click();
  }
  ...
  async contarTracks()
  {
    ...
    if(await rtaBool)
    {
      const rowTracks = await element.all(by.xpath('//*[@id="tackTable"]/table/tbody/tr')).count()
  ...
  async eliminarTrack()
  {
    ...
    const rowTracks = await element.all(by.xpath('//*[@id="tackTable"]/table/tbody/tr')).count()
    this.esperar(200)
    const tRow = await element.all(by.xpath('//*[@id="tackTable"]/table/tbody/tr'))
